...
impl<'a> Batch<'a> {
    pub fn insert(&self, relation: &str, tuple: Vec<Value>) -> Result<()> {
        let mut state = self.db.state.write().map_err(|_| anyhow!("lock poisoned"))?;
        state.store.insert(relation, tuple)?;
        state.store.merge_deltas();
        Ok(())
    }
...
fn extract_idb_snapshot(store: &dyn Store, idb_relations: &HashSet<String>) -> crate::backend::IdbSnapshot {
...
fn full_recompute(
    ...
    let interpreter =
        mangle_driver::execute(&mut ir, &stratified, Box::new(exec_store))?;
